...
inline void ApplyGate(Simulator& simulator, const Gate& gate,
 ...
 */
template <typename Simulator, typename Gate>
inline void ApplyGateDagger(Simulator& simulator, const Gate& gate,
                            typename Simulator::State& state) {
  if (gate.kind != gate::kMeasurement) {
...
inline bool ApplyGate(
    ...
    Simulator& simulator, const Gate& gate, Rgen& rgen,
    typename Simulator::State& state,
 ...
 */
template <typename Simulator, typename Gate, typename Rgen>
inline bool ApplyGate(typename Simulator::StateSpace& state_space,
 ...
 */
template <typename Simulator, typename Gate>
inline void ApplyFusedGate(Simulator& simulator, const Gate& gate,
                           typename Simulator::State& state) {
  if (gate.kind != gate::kMeasurement) {
 ...
 */
template <typename Simulator, typename Gate>
inline void ApplyFusedGateDagger(Simulator& simulator, const Gate& gate,
                                 typename Simulator::State& state) {
  if (gate.kind != gate::kMeasurement) {
    auto matrix = gate.matrix;
    MatrixDagger(unsigned{1} << gate.qubits.size(), matrix);
