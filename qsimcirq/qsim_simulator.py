...
class QSimSimulator(
        ...
        if self.qsim_options["g"]:
            if self.qsim_options["gmode"] == 0:
                ...
                if qsim_custatevec is None:
                    ...
                    print(f" Using cuStateVec > But I don't chose custatevec")
    ...
    def _sample_measure_results(
        ...
        print(f" number of Qubit: {num_qubits}")
        ...
        if not noisy and program.are_all_measurements_terminal() and repetitions > 1:
            ...
            #print(" translate_cirq_to_qsim start 2 ? ")
            measurements = np.empty(shape=(repetitions, num_bits), dtype=int)
            for i in range(repetitions):
                options["s"] = self.get_seed()
                measurements[i] = sampler_fn(options)
                #print(f" Current state vector for repetition {i}: {measurements[i]}")
    ...
    def _simulate_impl(
        ...
        # Add noise to the circuit if a noise model was provided.
        all_qubits = program.all_qubits()
        print(f" All qubits in the program: {all_qubits}")
        ...
        # qsim numbers qubits in reverse order from cirq
        cirq_order = cirq.QubitOrder.as_qubit_order(qubit_order).order_for(all_qubits)
        print(f" Cirq order of qubits: {cirq_order}")
        
        qsim_order = list(reversed(cirq_order))
        print(f" QSim order of qubits (reversed Cirq order): {qsim_order}")
        ...
        if isinstance(initial_state, np.ndarray):
            ...
            print(f" Allocated state vector data (input_vector): {input_vector}")
            '''
        ...
        for prs in param_resolvers:
            ...
            if isinstance(initial_state, int):
                print(f" Initial state as integer: {initial_state}")
                qsim_state = fullstate_simulator_fn(options, initial_state)
            elif isinstance(initial_state, np.ndarray):
                print(f" Initial state as ndarray: {input_vector}")
                qsim_state = fullstate_simulator_fn(options, input_vector)
            
            # Debugging output
            print(f" Initial_state: {initial_state}")
            print(f" Current state vector (qsim_state): {qsim_state}")
    ...
    def _translate_circuit(
        ...
        for original, translated, moment_indices in self._translated_circuits:
            if original == circuit:
                #print(" Memoized Translated Circuit:", translated)
                #print(" Memoized Moment Indices:", moment_indices)
        ...
        #print(" Translated Circuit:", translated, dir(translated))
        #print(" Qubit order:", qubit_order)
        #print(" Moment Indices:", moment_indices)
        
        #for gate in translated.gates:
        #    print(gate.to_string())

        #print(" Translated Circuit Gates:", translated.gates)
        #print(" Number of Qubits:", translated.num_qubits)
