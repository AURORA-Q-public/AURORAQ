...
Cirq::GateCirq<float> create_gate(const qsim::Cirq::GateKind gate_kind,
    ...
    // printf(" pybind_main.cpp-create_gate: gate_kind %llu\n");
...
Cirq::GateCirq<float> create_matrix_gate(const unsigned time,
  ...
  switch (qubits.size()) {
//  Not here
  printf(" pybind_main: create_matrix_gate: qubits.size() : %llu\n", qubits.size());
  case 0:
    // Global phase gate.
...
class SimulatorHelper {
 public:
  ...
  static py::array_t<float> simulate_fullstate(
    ...
    printf(" pybind_main.cpp: simulate_fullstate() \n");
 ...
 private:
  SimulatorHelper(const py::dict &options, bool noisy)
    ...
    try {
      ...
      StateSpace state_space = factory.CreateStateSpace();
      printf(" Pybind-CreateStateSpace() \n");
  ...
  void init_state(uint64_t input_state) {
    StateSpace state_space = factory.CreateStateSpace();
    printf(" Pybind(init_state) - CreateStateSpace() \n");
  ...
  bool simulate(const StateType& input_state) {
    printf(" pybind-simulate() - Call init_state\n");
  ...
  py::array_t<float> release_state_to_python() {
   // printf(" pybind_main.cpp release_state_to_python() start\n");
    StateSpace state_space = factory.CreateStateSpace();
   ...
   // printf(" fsv_size: %llu, fsv (address): %p, MinSize: %llu\n", fsv_size, (void*)fsv, min_size);
